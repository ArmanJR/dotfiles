...
finddupes() {
    ...
    if command -v fdupes >/dev/null 2>&1; then
        ...
        find "$directory" -type f -exec md5sum {} \; | sort | uniq -d -w 32 | cut -c 35-
    fi
}
...
speedtest() {
    if command -v speedtest-cli >/dev/null 2>&1; then
        ...
        echo "Install speedtest-cli with: uv tool install speedtest-cli"
    fi
}
...
resources() {
    echo "=== CPU Usage ==="
    grep 'cpu ' /proc/stat | awk '{usage=($2+$4)*100/($2+$4+$5)} END {printf "%.1f%% used\n", usage}'
...
pomodoro() {
    ...
    (sleep $((duration * 60)) && \
        if command -v notify-send >/dev/null 2>&1; then
            notify-send "Focus Timer" "Pomodoro complete!"
        fi && \
