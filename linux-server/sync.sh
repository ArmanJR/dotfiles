...
# Dotfiles sync script (Linux Server)
...
SERVER_DIR="$DOTFILES_DIR/linux-server"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --dry-run)
            ...
            ;;
        --help)
            echo "Dotfiles sync script (Linux Server)"
            echo ""
...
if [[ "$SYNC_ZSH" == true ]]; then
    collect_directory_changes "$SERVER_DIR/.zsh" "$HOME/.zsh" ".zsh"
fi

if [[ "$SYNC_ZSHRC" == true ]]; then
    collect_file_changes "$SERVER_DIR/.zshrc" "$HOME/.zshrc" ".zshrc"
fi
...
if [[ "$SYNC_PREK" == true ]]; then
    collect_directory_changes "$SERVER_DIR/.config/prek" "$HOME/.config/prek" ".config/prek"
fi

if [[ "$SYNC_TMUX" == true ]]; then
    collect_file_changes "$SERVER_DIR/.tmux.conf" "$HOME/.tmux.conf" ".tmux.conf"
fi

if [[ "$SYNC_BIN" == true ]]; then
    collect_directory_changes "$SERVER_DIR/.local/bin" "$HOME/.local/bin" ".local/bin"
fi

if [[ "$SYNC_CLAUDE" == true ]]; then
    collect_directory_changes "$SERVER_DIR/.claude" "$HOME/.claude" ".claude"
fi
