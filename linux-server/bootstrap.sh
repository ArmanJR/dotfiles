#!/usr/bin/env bash
# ============================================================================
# Linux Server Development Environment Bootstrap Script
#
...
readonly SERVER_DIR="$DOTFILES_DIR/linux-server"
...
link_dotfiles() {
    ...
    if [[ ! -d "$SERVER_DIR" ]]; then
        log WARNING "Dotfiles directory not found at $SERVER_DIR"
    ...
    for file in "${shell_files[@]}"; do
        if [[ -f "$SERVER_DIR/$file" ]]; then
            ln -sf "$SERVER_DIR/$file" "$HOME/$file"
    ...
    if [[ -d "$SERVER_DIR/.claude" ]]; then
        mkdir -p "$HOME/.claude"
        for file in "$SERVER_DIR/.claude/"*; do
            [[ -f "$file" ]] && ln -sf "$file" "$HOME/.claude/$(basename "$file")"
        done
        if [[ -d "$SERVER_DIR/.claude/commands" ]]; then
            mkdir -p "$HOME/.claude/commands"
            for file in "$SERVER_DIR/.claude/commands/"*; do
...
show_help() {
    cat << 'EOF'
Linux Server Development Environment Bootstrap
...
main() {
    ...
    echo -e "${CYAN}"
    echo "============================================="
    echo "   Linux Server Dev Environment Bootstrap"
    echo "============================================="
    echo -e "${NC}"
    ...
    echo "3. Run 'claude' to set up Claude Code authentication"
